...
- Worker init: `loadPackage('micropip')` → install the packages pinned in `/pyodide-requirements.txt` (generated at the repo level by `uv`; see "Python tooling: uv & ruff"). Persists across runs in the same problem session.
...
- `pyproject.toml` at repo root — declares `requires-python`, `[dependency-groups]` with `dev = ["pytest", "ruff", "ty"]` and `pyodide = ["pytest"]` (the exact set the worker also installs). Hosts `[tool.ruff]` and `[tool.ty]` config.
- `uv.lock` — committed.
- `tooling/pyodide-requirements.txt` — generated by `uv export --group pyodide --no-hashes -o tooling/pyodide-requirements.txt`. The build script copies it to `web/static/pyodide-requirements.txt` so the worker can `fetch()` it at init.
...
### micropip (browser-side)

Worker init fetches the same pinned list and hands it to `micropip.install()`:

```python
import micropip, pyodide.http
reqs = (await pyodide.http.pyfetch("/pyodide-requirements.txt")).string().splitlines()
await micropip.install([r for r in reqs if r.strip() and not r.startswith("#")])
```

Guarantees authors and the browser run the same pytest version — no "works on my machine" drift.
...
- `tooling/pyodide-requirements.txt` (generated by `uv export --group pyodide`)
...
- **uv / ruff / ty are native, not WASM**: they cannot be called from inside Pyodide. uv manages the host toolchain (authoring, CI, dep locking) and generates the pinned list that micropip consumes in the browser. Browser lint/format/type-check uses Astral's separate WASM bundles, not Pyodide — ty-wasm availability may trail ruff-wasm, so plan for host-only type-checking at first.
